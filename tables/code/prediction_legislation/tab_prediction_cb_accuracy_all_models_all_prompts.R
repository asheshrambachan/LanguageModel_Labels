...
tab_path <- file.path(tab_dir, "tab_prediction_cb_accuracy_all_models_all_prompts.tex")
...
data <- read.csv(data_path) %>%
  filter(
    Model %in% c("GPT-3.5", "GPT-4o", "GPT-5-mini", "GPT-5-nano")
  ) %>%
  mutate(Model = factor(
...
tab <- data %>% 
  ...
  pack_rows("House", 1, 8, italic=T, bold=F)  %>%
  pack_rows("Senate", 9, 16, italic=T, bold=F)
