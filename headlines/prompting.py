...
companies = pd.read_csv("sample.csv")
...
for index, row in companies.iterrows():
    ...
    base_a = get_response(content=company_content)
    ...
    for persona in personas:
        ...
        try: 
            personas_a.append(get_response(content=(persona + company_content)))
        except:
            personas_a.append(None)

    thought_q = []
    thought_a = []
    pt1, pt2 = company_content.split("Write", 1)[0], company_content.split("Write", 1)[1]
    for thought in thought_modifiers:
        thought_q.append(pt1 + thought + pt2 + explanation)
        try: 
            thought_a.append(get_response(content=(pt1 + thought + pt2 + explanation)))
        except:
