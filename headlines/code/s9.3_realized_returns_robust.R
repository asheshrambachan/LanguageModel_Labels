...
for (q in 1:length(questions)) {
  for (m in 1:length(model_levels)) {
    ...
    #           covariate.labels = magnitude_labels,
    #           dep.var.labels = "Realized Return 5 Day-Post Headline",
    #           model.names = TRUE,
    #           notes = obs_note)
...
write.csv(plot_results, "./data/step9_reg_results/{return_type}_returns_robust.csv")
