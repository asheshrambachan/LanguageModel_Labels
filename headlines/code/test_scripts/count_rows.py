...
for main_dir in main_dirs:
    ...
    for model_dir in model_dirs:
        ...
        for question_dir in ["q1", "q2", "q3", "q4", "q5"]:
            question_path = os.path.join(model_dir_path, question_dir)
            
            # Loop through each CSV file in the question subdirectory
            if os.path.exists(question_path):
                for csv_file in os.listdir(question_path):
                    if csv_file.endswith(".csv"):
                        file_path = os.path.join(question_path, csv_file)
                        
                        # Load the CSV file into a pandas DataFrame and count the rows
...
summary_df.to_csv("prompt_counts.csv", index=False)