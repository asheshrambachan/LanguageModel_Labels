...
obs_note <- paste("Observations: ", paste(n_list[1:7], collapse = ", "))

plot_regression_coefficients(reg_list, file_names, "1-day Post Headline Cumulative Returns Regression Coefficients with Confidence Intervals")
...
plot_regression_coefficients(reg_list, file_names, "5-day Post Headline Cumulative Returns Regression Coefficients with Confidence Intervals")
ggsave(filename = paste0("temp_figs/coefficients/", model, "/", return_type, "_robust_ses/", question, "/ret5_magnitude.jpeg"),
       units = "in", width = 8, height = 5)

# Create stargazer tables
stargazer(reg_list[1:7], type = "text", se = se_list[1:7], 
          title = "5 Day Post Headline Returns Regressed on Headline Type (with Magnitude)",
          covariate.labels = magnitude_labels,
          dep.var.labels = "Return FD1",
          model.names = TRUE,
          notes = obs_note)
...
plot_regression_coefficients(reg_list, file_names, "10-day Post Headline Cumulative Returns Regression Coefficients with Confidence Intervals")
...
# Create stargazer tables
stargazer(reg_list[1:7], type = "text", se = se_list[1:7], 
          title = "5 Day Post Headline Returns Regressed on Headline Type (with Confidence)",
          covariate.labels = confidence_labels,
          dep.var.labels = "Return FD1",
          model.names = TRUE,
          notes = obs_note)
