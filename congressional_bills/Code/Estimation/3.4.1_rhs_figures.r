...
fig05 <- data_5k %>%
  ...
  mutate(proportion = factor(proportion, levels=proportion_levels, labels=proportion_labels)) %>%
  ggplot(aes(
    x=bias_mean, 
  ...
  xlab("Bias") +
  ylab("Probability Densities") +
  facet_grid(Y ~ model + proportion, labeller=label_wrap_gen(width=16), scales = "free_x") +
  scale_fill_manual(name=NULL, values=my_colors) +
  scale_x_symmetric(mid=0) +
...
ggsave(fig_path, plot = fig05, height = 7, width = 9)
cat(sprintf("Saved %s\n", fig_path))
...
fig06 <- data_5k %>%
  ...
  xlab("Normalized Bias") +
  ylab("Probability Densities") +
  facet_grid(Y ~ model + proportion, labeller=label_wrap_gen(width=16)) +
  scale_fill_manual(name=NULL, values=my_colors) +
  scale_x_symmetric(mid=0) +
  scale_y_continuous(minor_breaks=seq(0,1, by=0.05), limits=c(0,1)) +
  theme.bar
...
ggsave(fig_path, plot = fig06, height = 7, width = 9)
cat(sprintf("Saved %s\n", fig_path))
...
fig07 <- data_5k %>%
  ...
  xlab("log(MSE)") +
  ylab("Empirical CDF") +
  facet_grid(Y ~ proportion, labeller=label_wrap_gen(width=16)) +
...
ggsave(fig_path, plot = fig07, height = 7, width = 9)
cat(sprintf("Saved %s\n", fig_path))
...
ggsave(fig_path, plot = fig08, height = 7, width = 9)
cat(sprintf("Saved %s\n", fig_path))
