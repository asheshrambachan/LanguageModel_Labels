...
fig_color <- my_colors[as.character(sort(unique(c(data_fig01$model, data_human$model))))]

fig01_frame1 <- fig01 + 
  scale_color_manual(name=NULL, values=alpha(my_colors, 0)) + 
  guides(color = guide_legend(override.aes = list(color=fig_color, shape = c(16, 17, NA))))
fig01_frame2 <- fig01

# save figure
ggsave(file.path(slides_fig_dir, "Coefficient vs. Prompt-Model, Sorted, Frame 1.jpeg"), plot = fig01_frame1, height = 4, width = 9)
ggsave(file.path(slides_fig_dir, "Coefficient vs. Prompt-Model, Sorted, Frame 2.jpeg"), plot = fig01_frame2, height = 4, width = 9)
...
# Create frames for the slides from the figure above
fig_color <- my_colors[as.character(sort(unique(data_fig02$model)))]

fig02_frame1 <- fig02 + 
  scale_color_manual(name=NULL, values=alpha(my_colors, 0)) + 
  guides(color = guide_legend(override.aes = list(color=fig_color, shape = c(16, 17))))
fig02_frame2 <- fig02

# save figure
ggsave(file.path(slides_fig_dir, "t-score vs. Prompt-Model, Sorted, Frame 1.jpeg"), plot = fig02_frame1, height = 4, width = 9)
ggsave(file.path(slides_fig_dir, "t-score vs. Prompt-Model, Sorted, Frame 2.jpeg"), plot = fig02_frame2, height = 4, width = 9)
...
# save figure
ggsave(file.path(slides_fig_dir, "Normalized Bias Density by Model, Val Prop 10, LLM and Debiased, Frame 1.jpeg"), plot = fig06_prop10_frame1, height = 4, width = 8)
ggsave(file.path(slides_fig_dir, "Normalized Bias Density by Model, Val Prop 10, LLM and Debiased, Frame 2.jpeg"), plot = fig06_prop10_frame2, height = 4, width = 8)
