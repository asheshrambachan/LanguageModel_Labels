...
rhs_dir <- file.path(repo_dir, "estimation_legislation/temp/RHS")
dir.create(data_dir, showWarnings=FALSE, recursive = TRUE)

# Data paths
rds_paths <- list.files(rhs_dir, pattern="*.rds", full.names=TRUE)
data_bills_path <- file.path(data_dir, "bills.csv")
plugin_data_path <- file.path(data_dir, "rhs_10k_plugin.csv")
validation_data_path <- file.path(data_dir, "rhs_10k_validation.csv")
plugin_validation_debiased_averaged_data_path <- file.path(data_dir, "rhs_5k_plugin_validation_debiased_averaged.csv")
summary_stat_10k_path <- file.path(data_dir, "rhs_10k_plugin_summary_stat.csv")
summary_stat_5k_path <- file.path(data_dir, "rhs_5k_plugin_validation_debiased_averaged_summary_stat.csv")
...
regression_labels_levels <- c(
  "10k_V_Yllm"="10k Plug-In", 
  "10k_V_Yhuman"="10k Validation",
  "5k_V_Yllm"="Plug-In", 
...
data <- bind_rows(lapply(rds_paths, readRDS)) %>% 
  ...
  select(!class) %>%
  mutate(
    model = if_else(model=="gpt-4o", "gpt-4o-2024-05-13", model),
...
coef_sample_average_data <- read.csv(data_bills_path) %>%
  ...
  tidyr::pivot_longer(cols = everything(), names_to = "W", values_to = "Sample Average")

# Merge and save summary_stat_10k
summary_stat_10k <- merge(plugin_summary_stat_data, coef_sample_average_data, by="W") %>%
  arrange(W, V)
write.csv(summary_stat_10k, summary_stat_10k_path, row.names=FALSE)
log_info("Saved {summary_stat_10k_path}")


# Save rhs_10k_validation.csv
validation_data <- data %>% 
  filter(regression=="10k Validation") %>%
  select(V, W, coef, t, lci, uci) %>%
  distinct()
...
plugin_validation_debiased_averaged_data <- plugin_validation_debiased_data %>% 
  group_by(combination_id, proportion, model, prompt, V, W, regression) %>% 
  summarise(
    coef_mean = mean(coef),
    coef_sd = sd(coef), 
    bias_mean = mean(bias), 
