...
TEMP_DIR = os.path.join(REPO_DIR, "estimation_legislation/temp/llm")
...
def create_prompts(prompting_strategies, bills, min_confidence=0.9, max_confidence=1):
    ...
    for _, bill in bills.iterrows():
        for _, strategy in prompting_strategies.iterrows():
            ...
            # add random confidence values for the example_assistant content
            if (strategy["AddExamples"]):
                example_assistant_ids = [1,3,5,7,9]
                for assistant_id in example_assistant_ids:
                    confidence = np.random.uniform(min_confidence, max_confidence)
                    bill_messages[assistant_id]["content"] = bill_messages[assistant_id]["content"].format(np.round(confidence, decimals=2)) 
