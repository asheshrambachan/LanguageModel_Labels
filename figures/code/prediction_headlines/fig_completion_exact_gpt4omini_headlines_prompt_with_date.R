# Figure: Examples of GPT-4o-mini completions with date restriction that exactly match original financial news headlines.
# Dec 10, 2024
...
fig_path <- file.path(fig_dir, "fig_completion_exact_gpt4omini_headlines_prompt_with_date.tex")
...
data <- read.csv(data_path) %>%
  ...
  rename(
    ...
    "GPT-4o-mini" = headline_llm_clean
  ) 
...
fig <- apply(data_sample, 1, function(row) {
  create_latex_row(row["Original Headline"], row["GPT-4o-mini"])
}) %>%
