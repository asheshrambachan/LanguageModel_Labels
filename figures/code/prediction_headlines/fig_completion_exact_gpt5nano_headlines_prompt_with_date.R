# Figure: Examples of GPT-5-nano completions with date restriction that exactly match original financial news headlines.
# Dec 10, 2024
...
fig_path <- file.path(fig_dir, "fig_completion_exact_gpt5nano_headlines_prompt_with_date.tex")
...
fig <- apply(data_sample, 1, function(row) {
  create_latex_row(row["Original Headline"], row["GPT-5-nano"])
}) %>%
