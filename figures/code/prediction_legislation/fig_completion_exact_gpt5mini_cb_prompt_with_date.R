...
fig_path <- file.path(fig_dir, "fig_completion_exact_gpt5mini_cb_prompt_with_date.tex")
...
data <- read.csv(data_path) %>%
  ...
  rename(
    ...
    "GPT-5-mini" = DescriptionLLMClean
  )
...
fig <- apply(data_sample, 1, function(row) {
  create_latex_row(row["Original Bill"], row["GPT-5-mini"])
}) %>%
