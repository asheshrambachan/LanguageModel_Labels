# Prediction Tasks: Congressional Legislation

This subdirectory contains scripts, data, and outputs for **prediction tasks** on **Congressional legislation**. The steps involves cleaning and preparing the data, querying LLMs, generating embeddings for similarity analysis, and summarizing the results for use in figures and tables.
...
2. **Partial Replication:** Run individual scripts for specific steps. 
    ...
    1. Data cleaning.
        ```
        python ./prediction_legislation/code/1_clean_data.py
        ```

    2. Prompt creation, LLM querying, and response decoding. 
        ```sh
        python ./prediction_legislation/code/2.1_create_prompts.py
        python ./prediction_legislation/code/2.2_query_llm.py
        python ./prediction_legislation/code/2.3_download_responses.py
        python ./prediction_legislation/code/2.4_decode_responses.py
        Rscript ./prediction_legislation/code/2.5_summarize_prediction.R
        ```

    3. Embedding generation and similarity analysis:
        ```sh
        python ./prediction_legislation/code/3.1_create_embed_prompts.py
        python ./prediction_legislation/code/3.2_query_embed_prompts.py
        python ./prediction_legislation/code/3.3_download_embed_responses.py
        python ./prediction_legislation/code/3.4_decode_embed_responses.py
        Rscript ./prediction_legislation/code/3.5_summarize_completion.R
        ```
